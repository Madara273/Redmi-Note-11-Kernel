	...
	while (true) {
		...
		if (injection_ctx->wma_handle) {
			...
			if (monitor_mode_active) {
				...
				if (soc) {
					vdev_ptr = cdp_get_mon_vdev_from_pdev(soc,
									 OL_TXRX_PDEV_ID);
					if (vdev_ptr) {
						struct ol_txrx_vdev_t *vdev = (struct ol_txrx_vdev_t *)vdev_ptr;
						cdp_mon_vdev_id = (uint8_t)vdev->vdev_id;
					} else {
						cdp_mon_vdev_id = 0xff;
					}
				}
