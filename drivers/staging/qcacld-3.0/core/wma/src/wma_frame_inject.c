 ...
 */
#define WMA_INJECTION_DEBUG_CACHE_SIZE 512
					  ...
					  uint8_t vdev_id)
{
	...
	if (!wmi_tx_ok) {
...

		{
			...
			tp_wma_handle wma_ptr = cds_get_context(QDF_MODULE_ID_WMA);

			vdev_handle = wma_find_vdev_by_id(wma_ptr, mgmt_params.vdev_id);
			if (vdev_handle) {
				ret = cdp_mgmt_send_ext(soc,
							vdev_handle,
							wmi_buf,
							mgmt_params.tx_type,
				...
				wma_err("Failed to find vdev handle for id %d", mgmt_params.vdev_id);
				qdf_nbuf_free(wmi_buf);
				return QDF_STATUS_E_INVAL;
			}
		}
